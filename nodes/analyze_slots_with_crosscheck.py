...
写作要求：
- 创新性：对于每个槽位填入的内容，思考这里能体现什么独创性
- 技术实现难度与完整性：技术描述要有深度，展示技术选型的合理性
- 应用价值：解决什么实际问题，有何推广价值
- 引用：遵守引用规范，在所有生成的段落中检查是否标明所有参考出处。
- 报告质量：逻辑清晰，术语规范
...
请严格遵守以下输出格式，在output标签内严格遵循json格式，不要有多余字符:

<think> （你的思考推理过程） </think>
<output>
{
  "slots": [
    {
...
请严格遵循以下输出格式，在output标签内严格遵循json格式，不要有多余字符：
<think> (你的思考过程) </think>
<output>
{
  "slots": [
...
def _extract_output_block(raw: str) -> str:
    """优先提取 <output>...</output> 中的内容，未命中则返回原文。"""
...
def _safe_json(text: str) -> Dict[str, Any]:
    ...
    # 2) 优先提取 <output> 标签内容（兼容 <think>/<output> 格式）
